...
# ------------------------------------------------------------
# PURPOSE:
# Define all person-night time windows used to assign exposure data.
#
# IMPORTANT:
# The fixed nocturnal window is aligned with the earlier HTML/REHEAT
# preprocessing:
# 23:00-07:00, implemented as [23:00, 08:00).
# This includes hourly observations 23, 00, 01, ..., 07.
# ------------------------------------------------------------
...
sleep_windows <- sleep_diary %>%
  ...
  dplyr::transmute(
    ...
    # Fixed nocturnal exposure window:
    # 23:00 on sleep_date to 08:00 on wake_date, with 08:00 excluded.
    # This includes hourly weather observations 23:00 through 07:00.
    night_start_fixed =
      as.POSIXct(
        paste0(
          sleep_date,
    ...
    # Daytime mEDI window starts at 06:00 on the sleep_date.
    medi_day_start =
      as.POSIXct(
        paste0(
          sleep_date,
    ...
    # Evening mEDI window covers the last 4 hours before sleep preparation.
    medi_evening_start =
      pre_sleep_anchor - lubridate::hours(
        4
      ),
    ...
    # Daytime mEDI ends where the evening window starts.
    medi_day_end =
      medi_evening_start
  ) %>%
  dplyr::filter(
    ...
    night_end_fixed > night_start_fixed
  )
# ------------------------------------------------------------
...
weather_hourly_raw <- tryCatch(
  {
    readxl::read_excel(
      ...
      # Treat text-coded missing values as actual missing values.
      na =
        c(
          "",
          "NA",
...
# ------------------------------------------------------------
# STEP 13a: Prepare hourly weather data and save updated Finca file
# ------------------------------------------------------------
# PURPOSE:
# Keep valid hourly weather records, save an updated copy of the
# Finca Excel file with wet-bulb temperature, and document the
# imported weather variables.
#
# IMPORTANT:
# The updated Finca file is saved next to the original Excel file,
# not hidden in output/preprocessing.
# ------------------------------------------------------------
...
message(
  "Saving updated Finca weather file to: ",
  normalizePath(
    weather_with_wetbulb_file,
    winslash = "/",
    mustWork = FALSE
  )
)
...
weather_variable_check <- tibble::tibble(
  ...
  n_missing_datetime =
    sum(
      is.na(
        weather_prepared_all$datetime
      )
    ),
  
  n_missing_temperature_c =
    sum(
      is.na(
        weather_prepared_all$temperature_c
      )
    ),
  
  n_missing_rh_percent =
    sum(
      is.na(
        weather_prepared_all$rh_percent
      )
    ),
  
  n_missing_twb_stull_c =
    sum(
      is.na(
        weather_prepared_all$twb_stull_c
      )
...
# ------------------------------------------------------------
# STEP 13b: Aggregate nightly weather using the REHEAT/HTML night definition
# ------------------------------------------------------------
# PURPOSE:
# Aggregate Finca weather exposure using the same nocturnal window
# as the earlier preprocessing.
#
# NIGHT DEFINITION:
# 23:00-07:00 local time, including the 07:00 observation.
#
# IMPLEMENTATION:
# Included hourly timestamps are:
# 23, 00, 01, 02, 03, 04, 05, 06, 07.
#
# This is equivalent to the half-open interval [23:00, 08:00).
#
# DATE ASSIGNMENT:
# - 23:00 belongs to the same sleep_date.
# - 00:00-07:00 are assigned to the previous sleep_date.
#
# OUTPUT:
...
include_07_hour <- TRUE

expected_nocturnal_hours <- ifelse(
  include_07_hour,
  9L,
  8L
)

min_required_nocturnal_hours <- ifelse(
  include_07_hour,
  6L,
  5L
)
...
# ------------------------------------------------------------
# Define nocturnal weather-window settings
# ------------------------------------------------------------
# PURPOSE:
# Align weather preprocessing with the earlier HTML/REHEAT definition:
# 23:00-07:00, including the 07:00 observation.
# This corresponds to expected hours: 23, 0, 1, 2, 3, 4, 5, 6, 7.
# ------------------------------------------------------------

include_07_hour <- TRUE

expected_nocturnal_hours <- 9L

min_required_nocturnal_hours <- 6L
...
weather_nocturnal_hourly <- weather_prepared %>%
  dplyr::mutate(
    # Keep date and hour in Costa Rica local time.
    datetime_local =
      lubridate::with_tz(
        datetime,
        tzone =
    ...
    is_nocturnal_23_07 =
      dplyr::case_when(
        include_07_hour ~
          weather_hour >= 23 | weather_hour <= 7,
...
weather_night_by_date <- weather_nocturnal_hourly %>%
  dplyr::group_by(
    sleep_date
  ) %>%
  dplyr::summarise(
    ...
    first_weather_datetime_night =
      min(
        datetime,
        na.rm = TRUE
      ),
    
    last_weather_datetime_night =
      max(
        datetime,
        na.rm = TRUE
      ),
    ...
    nocturnal_weather_qc =
      dplyr::case_when(
        n_temperature_records_night >= min_required_nocturnal_hours &
          n_rh_records_night >= min_required_nocturnal_hours &
          n_twb_records_night >= min_required_nocturnal_hours ~
          "sufficient",
...
weather_night_check <- weather_night_by_date %>%
  dplyr::summarise(
    n_sleep_dates_with_weather =
      dplyr::n(),
    
    include_07_hour =
      include_07_hour,
    
    expected_nocturnal_hours =
      expected_nocturnal_hours,
    
    min_required_nocturnal_hours =
      min_required_nocturnal_hours,
    ...
    n_complete_23_07_nights =
      sum(
        n_weather_records_night == expected_nocturnal_hours,
        na.rm = TRUE
      ),
    
    prop_complete_23_07_nights =
      mean(
        n_weather_records_night == expected_nocturnal_hours,
        na.rm = TRUE
      ),
    
    n_sufficient_weather_nights =
      sum(
        nocturnal_weather_qc == "sufficient",
        na.rm = TRUE
      ),
    
    prop_sufficient_weather_nights =
      mean(
        nocturnal_weather_qc == "sufficient",
        na.rm = TRUE
      ),
...
weather_hour_pattern_check <- weather_night_by_date %>%
  dplyr::count(
    n_weather_records_night,
    weather_hours_night,
    nocturnal_weather_qc,
